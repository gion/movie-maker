...
movieMakerApp.controller('MainCtrl', ['$scope', 'config', 'util', '$http' ,'$timeout', '$window',
function($scope, config, util, $http, $timeout, $window) {
...
/*	$(window).load(function(){
	...
	$http.get(config.demoUrl).success(function(data){
		...
		$window.T = $scope.VisualTimeline = new Timeline($scope.timelines.visual.concat($scope.timelines.audio).concat($scope.timelines.music));
	...
	function Timeline(elements){
		...
		this.tweens = [];
		this.timeline = null;
		this.output = angular.element('#screen');
	...
	angular.extend(ImageItem.prototype, {
		...
		initOutputElement : function(){
			this.output = angular.element('<img src="'+ this.data.source +'" alt="" class="item image-item" />');
		},
		onStart : function(){
